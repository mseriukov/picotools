public struct REVSH: Instruction {
    enum Kind {
        case REVSH(Register, Register)
    }
    ...
    public init(_ desc: InstructionDescriptor) throws {
        self.desc = desc
        guard desc.mnemonic == .REVSH else { fatalError("Mnemonic doesn't match the expected one.") }
        ...
        self.kind = .REVSH(r1, r2)
    }

    public func encode(symbols: [String: UInt16]) throws -> [UInt16] {
        switch kind {
        case let .REVSH(r1, r2):
            return Thumb.REVSH(d: r1.number, m: r2.number).encode()
        }
    }
}
